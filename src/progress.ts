...
export function loadProgress():Progress{
 try{
  ...
  if(raw)return{...emptyProgress,...JSON.parse(raw),cases:{...(JSON.parse(raw).cases||{})}};
...
export function recordAttempt(code:string,score:number,baseIq:number,correct:boolean):AttemptUpdate{
 ...
 // IQ is case mastery, not a repeatable currency: only the positive delta over the best reward already earned is added.
