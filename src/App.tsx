...
import {ArrowRight,Beaker,Check,ChevronLeft,ClipboardCheck,Clock3,Eye,FlaskConical,Trophy,Flame,Target,ChartNoAxesCombined} from 'lucide-react';
...
export default function App(){
 ...
 return <main className="workspace"><header className="work-head"><button className="icon" onClick={()=>setScreen('brief')}><ChevronLeft/></button><div><span>STEP {guideStep} OF 3</span><b>{c.title}</b></div><div className="budget"><small>BUDGET LEFT</small><strong>{c.budget-spent}</strong></div></header><div className="work-grid" style={{gridTemplateColumns:'minmax(0,760px)',justifyContent:'center'}}>
 {guideStep===1&&<section className="column" style={{borderRight:0}}><Title icon={<Eye/>} title="Step 1 — Check the clues" sub="Tap each clue to mark it as reviewed"/><div className="observation"><span>WHAT YOU SEE</span><b>{c.observation}</b><p>Start here, then open the clues below.</p></div>{c.evidence.filter(e=>e.initial).map(e=><button key={e.id} onClick={()=>review(e.id)} className={'evidence '+(reviewed.includes(e.id)?'seen':'')}><div><Eye size={16}/><b>{e.title}</b>{reviewed.includes(e.id)&&<Check size={15}/>}</div><p>{e.detail}</p></button>)}<button className="primary" style={{width:'100%',marginTop:10}} disabled={!reviewedAvailable} onClick={()=>setGuideStep(2)}>Next: run tests <ArrowRight size={17}/></button>{!reviewedAvailable&&<small className="warning">Open all available clues first.</small>}</section>}
 {guideStep===2&&<section className="column" style={{borderRight:0}}><Title icon={<FlaskConical/>} title="Step 2 — Run useful tests" sub={`Budget used ${spent}/${c.budget}`}/><div className="panel" style={{marginBottom:12}}><p style={{margin:0,color:'#9ca49d',lineHeight:1.55}}>Tests can reveal hidden clues. You do not need to run every test. Try to spend your budget only on tests that help rule causes in or out.</p></div>{c.tests.map(t=>{const done=tests.includes(t.id),cant=!done&&spent+t.cost>c.budget;return <div className="test" key={t.id}><div><b>{t.title}</b><span><Clock3 size={13}/>{t.cost} budget points</span></div><button disabled={cant||done} onClick={()=>runTest(t.id)}>{done?'Done':cant?'Too expensive':'Run test'}</button>{done&&<p className="test-result"><Check size={14}/>{t.result}</p>}</div>})}<div style={{display:'flex',gap:10,marginTop:14,flexWrap:'wrap'}}><button className="ghost" onClick={()=>setGuideStep(1)}>Back to clues</button><button className="primary" onClick={()=>setGuideStep(3)}>Next: choose a cause <ArrowRight size={17}/></button></div></section>}
 {guideStep===3&&<section className="column" style={{borderRight:0}}><Title icon={<Beaker/>} title="Step 3 — Choose the cause" sub="Pick the explanation best supported by the clues and tests"/>{c.hypotheses.map(h=><button className={'hyp '+(hyp===h.id?'selected':'')} onClick={()=>setHyp(h.id)} key={h.id}><span className="radio">{hyp===h.id&&<i/>}</span><div><b>{h.title}</b><p>{h.description}</p></div></button>)}<div style={{display:'flex',gap:10,marginTop:14,flexWrap:'wrap'}}><button className="ghost" onClick={()=>setGuideStep(2)}>Back to tests</button><button className="submit" style={{flex:1,marginTop:0}} disabled={!hyp} onClick={submit}>Submit answer <ArrowRight size={17}/></button></div><small className="warning">After submitting, the game will explain the correct reasoning.</small></section>}
 </div></main>;
}
...
function Subject({c}:{c:FailureCase}){return <div>{c.subject}</div>}
